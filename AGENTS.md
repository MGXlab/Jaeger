...
- Contig keys are the first whitespace-delimited token of each FASTA header (single-word headers, e.g. `contig_1`), normalized as `.strip().replace(" ", "___").replace(",", "#")`. `fragment_generator` emits this key and every downstream FASTA re-read (prophage report, boundary refinement, plots, terminal-repeat scan) must look up the same key — never use `pyfastx full_name=True` and never silently rename duplicates with `_N` suffixes. Duplicate keys are rejected with an error in `validate_fasta_entries` before inference starts.
