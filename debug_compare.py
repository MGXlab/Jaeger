...
WEIGHTS_PATH = PROJECT_YAML.parent / "jaeger_d1754a4e_3.4M_fragment.weights.h5"
...
# The original training CSV is not available on this machine, so replace the
# data-dependent bias initializer with a constant zero initializer.  The weights
# file still contains the learned bias, so loading it afterward restores the
# correct value.
...
# Read first validation sequence
label = None
seq = None
...
# Built model (Keras) inference
print("\n--- Built Keras classifier ---")
...
logits_from_emb_built = models["classification_head"](emb_built, training=False)
...
print("\n--- Differences ---")
print(
    "max |built_logits - saved_logits|:",
    np.max(np.abs(logits_built.numpy() - logits_saved.numpy())),
)
print(
    "max |built_softmax - saved_softmax|:",
    np.max(np.abs(tf.nn.softmax(logits_built).numpy() - tf.nn.softmax(logits_saved).numpy())),
)
if emb_saved is not None:
    print(
        "max |built_embedding - saved_embedding|:",
        np.max(np.abs(emb_built.numpy() - emb_saved.numpy())),
    )
print(
    "max |logits_from_emb_built - logits_built|:",
    np.max(np.abs(logits_from_emb_built.numpy() - logits_built.numpy())),
)

# Test whether the SavedModel was exported under mixed_float16 while the
# current builder uses fp32.
...
classifier16.load_weights(WEIGHTS_PATH)
logits_built16 = classifier16(inputs_batch, training=False)
rep_out_built16 = models16["rep_model"](inputs_batch, training=False)
emb_built16 = rep_out_built16[0]
print("built16 logits:", logits_built16.numpy().flatten())
print("max |built16_logits - saved_logits|:", np.max(np.abs(logits_built16.numpy() - logits_saved.numpy())))
if emb_saved is not None:
    print(
        "max |built16_embedding - saved_embedding|:",
        np.max(np.abs(emb_built16.numpy() - emb_saved.numpy())),
    )

# Test whether the SavedModel is using batch statistics (training=True behavior)
...
print("built_train logits:", classifier(inputs_batch, training=True).numpy().flatten())
